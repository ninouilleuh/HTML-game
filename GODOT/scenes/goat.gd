...
func _process(delta):
	...
	if position.distance_to(player.position) < 20.0:
		...
		if main and main.has_method("game_over"):
			...
			if game_over_scene:
				...
				print("[GOAT DEBUG] GameOverScreen.tscn not found at res://scenes/GameOverScreen.tscn!")
	...
	if demonic:
		...
		if int(demonic_timer) != int(demonic_timer - delta):
			print("[GOAT DEBUG] Demonic timer: %.1f / %.1f seconds" % [demonic_timer, demonic_duration])
			...
			if has_node("Sprite2D"):
				...
				for child in get_children():
					if child is Sprite2D:
						...
						child.visible = true
						break
			print("[GOAT DEBUG] Demonic goat reverted to normal after timer expired at %s" % str(position))
			return
		...
		if current_tile.distance_to(player_tile) <= demonic_chase_distance:
			...
			print("[GOAT DEBUG] Demonic goat chasing player at %s (vibrating)" % str(position))
			return
		...
		if not moving:
			...
			if move_timer <= 0.0:
				...
				if valid_moves.size() == 0:
					...
					print("[GOAT DEBUG] Demonic goat: No valid moves from %s, waiting (vibrating)" % str(current_tile))
					# Vibrate in place
				...
				print("[GOAT DEBUG] Demonic goat: Wandering to %s from %s" % [str(target_tile), str(current_tile)])
				return
			...
			if position.distance_to(target_pos) <= distance:
				...
				print("[GOAT DEBUG] Demonic goat: Arrived at wander target %s (vibrating)" % str(target_tile))
			else:
				position += direction * distance
				# Add vibration to position
				position.x += sin(demonic_vibrate_timer) * demonic_vibrate_amount
				print("[GOAT DEBUG] Demonic goat: Moving toward wander target %s from %s (vibrating)" % [str(target_tile), str(position)])
			return
	...
	if chasing:
		has_chased = true
		print("[GOAT DEBUG] Chasing player: dist %d, at %s" % [dist_to_player, str(position)])
	...
	if not chasing and has_chased:
		if not demonic_pending and not demonic:
			...
			if randi() % 100 < 30:
				...
				demonic_pending_timer = 0.0
				moving = false
				print("[GOAT DEBUG] 30% chance triggered: Starting demonic pending at %s (lost player after chase)" % str(position))
			else:
	...
	if chasing:
		...
		print("[GOAT DEBUG] Moving toward player at %s" % str(position))
		return
	...
	if not moving:
		...
		if move_timer <= 0.0:
			...
			if valid_moves.size() == 0:
				move_timer = move_delay * randf()
				print("[GOAT DEBUG] No valid moves from %s, waiting" % str(current_tile))
				return
			...
			if preferred_moves.size() > 0:
				print("[GOAT DEBUG] Wandering: picked mountain-adjacent tile %s from %s" % [str(target_tile), str(current_tile)])
			else:
				print("[GOAT DEBUG] Wandering: picked random tile %s from %s" % [str(target_tile), str(current_tile)])
	else:
		var target_pos = tilemap.map_to_local(target_tile)
		var direction = (target_pos - position).normalized()
		var distance = speed * delta
		if position.distance_to(target_pos) <= distance:
			position = target_pos
			moving = false
			print("[GOAT DEBUG] Arrived at wander target %s" % str(target_tile))
		else:
			position += direction * distance
			print("[GOAT DEBUG] Moving toward wander target %s from %s" % [str(target_tile), str(position)])
